...
function is_remind(item,isRemind){
	...
	var vm = ff.page.vm();
	var activityCommodityId=item.activityCommodityId;
...
function timer(){
	...
	if(vm!=null && vm.obj!=null && vm.obj.activityGoods!=null ){
		if( vm.obj.activityGoods.leftSecond!=null &&  vm.obj.activityGoods.isPanicbuy=='1' && vm.obj.activityGoods.leftSecond>0 ){
			var leftSecond=vm.obj.activityGoods.leftSecond;
			//leftSecond
			 if(document.getElementById("panic-buying")!=null){
				if( ts<=-1)
					ts=leftSecond;
				 ...
				 ts--;
			     if(hh<=0 && mm<=0 && ss<=0){
			    	 document.getElementById("panic-buying").innerHTML = "";  
			         
			     }else{
			    		 document.getElementById("panic-buying").innerHTML = "<p>距结束：<span>"+  hhval + "</span>小时<span>" + 
...
function setRedis()
{
	...
	ff.page.submit({
 		...
 		condition:{goodsId:vm.obj.goods.goodsId,cityId:vm.address_id},
 		success:function(rsp)
 		{
